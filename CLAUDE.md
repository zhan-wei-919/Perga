...
Perga 是一个 Rust core 驱动的终端项目。当前仓库保留 PTY、SSH、终端协议解析、session、server 和 CLI 等核心能力;Web/Tauri 客户端已移除,后续客户端走各平台原生实现。跨平台方向见 [`docs/cross-platform-native-client.md`](docs/cross-platform-native-client.md)。

Rust core 是终端状态的事实来源,客户端不解析终端字节流。
...
- 调用方通过 channel / IPC 接收事件即可,不需要把核心 PTY / 引擎路径放进 async runtime。
- 避开 `Send + 'static` over `.await` 的 borrow checker 摩擦。

允许的例外:

- IPC / server handler 可以写成 `async fn`,内部通过 channel 与 PTY 线程通信。
...
TODO.md 不是 phase 规划 —— 方向性设计放在 `docs/` 的设计文档里;TODO.md 只放「当前代码里明知不完整、将来要补」的具体条目。
