//! SSH 连接参数。
//!
//! 本 crate **不知道** profile 的概念:`perga-core::profiles` 负责把
//! 用户的 `~/.perga/hosts.toml` 翻译成 [`SshConfig`]。这样桌面和移动端可以
//! 复用同一份 SSH 实现,不绑定 server 端的 profile schema。
...
///   不可用**。
/// - `Password`:明文密码,由客户端表单填,持久化在 server 端 `~/.perga/hosts.toml`
